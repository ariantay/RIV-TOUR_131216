...
*/	
    ...
    route: function() {
        ...
        if (!hash) {
			this.store.findByName('', function(employees) {
				$('#middle').html(HomeView.liTemplate(employees));
			});	
		}	
	...
	onSuccess: function (position) {
		...
		if (position.distance - distance<1.5) location.href="index.html#employees/" + position.id;		
		/*
	...
	updateDistance: function() {
		...
		for (var i=0; i < this.numStatues; i++) {
			...
			if (this.store.employees[i].distance - distance<1.5) location.href="index.html#employees/" + this.store.employees[i].id;
		}
    ...
    initialize: function() {
		...
		//var watchID = null;
        var self = this;
        this.detailsURL = /^#locationDetails\/(\d{1,})/;
        this.registerEvents();
